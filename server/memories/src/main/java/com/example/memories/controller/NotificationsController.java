...
public class NotificationsController {
    ...
    public ResponseEntity getNotificationById(@PathVariable @Min(value = 1, message = "Id must be greater than or equal to 1") Long id, BindingResult result) throws NotificationNotFoundException {
        ...
        Notifications notification = notificationService.getNotiById(id);
    ...
    public ResponseEntity createNotification(@PathVariable Long userId, Long postId, @Valid @RequestBody Notifications notification, BindingResult result)
    {
        if (result.hasErrors()){
