...
public class SearchRecentsController {
    ...
    public ResponseEntity<List<SearchRecents>> getAllSearchRecentsByUserId(@PathVariable Long userId, BindingResult result) throws SearchRecentNotFoundException{
        if (result.hasErrors()){
            throw new InvalidRequestException("Invalid Request Exception", result);
        }
    ...
    public ResponseEntity getSearchRecentById(@PathVariable @Min(value = 1, message = "Id must be greater than or equal to 1") Long id, BindingResult result) throws SearchRecentNotFoundException {
        if (result.hasErrors()){
    ...
    public ResponseEntity updateSearch(@PathVariable Long userId, @RequestBody SearchRecents searchRecents, BindingResult result) throws SearchRecentNotFoundException{
        if (result.hasErrors()){
